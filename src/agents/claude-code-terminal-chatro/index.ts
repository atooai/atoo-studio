...
export class ClaudeCodeTerminalChatROAgentFactory implements AgentFactory {
  ...
  async ownsSession(uuid: string): Promise<boolean> {
    // This agent shares the same session file format as the terminal agent.
    // Ownership is ambiguous — return false so the terminal agent claims it.
